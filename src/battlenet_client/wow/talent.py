...
class TalentAPI:
    ...
    def talent(self, locale: str, talent_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        if talent_id is not None:
            data: Dict[str, Any] = self.client.game_data(locale, 'static', 'talent', talent_id)
        else:
            data = self.client.game_data(locale, 'static', 'talent', 'index')
        return data

    @verify_client
    def pvp_talent(self, locale: str, pvp_talent_id: Optional[int] = None) -> Dict[str, Any]:
        """Returns an index of spells, or a specific spell
        ...
        if pvp_talent_id is not None:
            data: Dict[str, Any] = self.client.game_data(locale, 'static', 'pvp-talent', pvp_talent_id)
        else:
