...
from wow.exceptions import WoWClientError

MODULES = ['achievement', 'account', 'auction', 'azerite', 'character', 'connected_realm', 'covenant', 'creature',
           'guild', 'item', 'journal', 'media', 'modcraft', 'mount', 'mythic_plus', 'mythic_raid', 'pet',
           'playable_class', 'playable_race', 'playable_spec', 'power', 'profession', 'pvp', 'quest',
           'realm', 'region', 'reputation', 'spell', 'talent', 'tech_talent', 'title', 'wow_token']


class WoWClient(BattleNetClient):
    ...
    def __init__(self, region: str, *, release: str = 'retail',  scope: Optional[List[str]] = None,
                 redirect_uri: Optional[str] = None, client_id: Optional[str] = None,
                 client_secret: Optional[str] = None) -> None:

        super().__init__(region, WOW, client_id=client_id, client_secret=client_secret, scope=scope,
    ...
    def protected_data(self, locale: str, namespace: str, *args: Union[str, int]) -> Any:
        ...
        return self._post(uri, params={'namespace': getattr(self, namespace), 'locale': locale})
